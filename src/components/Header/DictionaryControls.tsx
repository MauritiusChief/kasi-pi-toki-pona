...
export function DictionaryControls() {
  ...
  const { currentDictionaryId, setCurrentDictionaryId, setContextDictionary } =
    useDictionaryContext();

  const nextDictionary = useMemo(() => {
    const currentIndex = AVAILABLE_DICTIONARIES.findIndex(
      (dictionary) => dictionary.id === currentDictionaryId,
    );
  ...
  return (
    <div
      ...
      data-testid="dictionary-controls"
    >
      ...
      <button
        ...
        title={
          nextDictionary ? `切换到 ${nextDictionary.name}` : "暂无其他字典"
        }
        aria-label={
          nextDictionary
